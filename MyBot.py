    ...
    if message.content.startswith('!help'):
        ...
        try:
            ...
            if not api_response.data.url:
                Rgifurl = 'Sry i cant find a random gif with the tag: ' + \
                    args[1]
            else:
        ...
        if len(args) < 2:
            ...
            TextToPrint = "Detta citat har nu lagts till: **" + \
                args[1] + "**"
        ...
        parsedData = json.dumps(
            DataToAdd, indent=4, sort_keys=True, ensure_ascii=False).encode('utf-8')
        ...
        if len(args) < 2:
            ...
            if (len(EditArgs) < 2):
                ...
                TextToChange = "Nu är " + \
                    EditArgs[0] + " ändrat till: " + EditArgs[1]
        ...
        if (len(args) < 2):
            ...
            try:
                ...
                cryptoText = cryptoName + ' är just nu värd ' + \
                    cryptoValue[0]['price_eur'] + ' EUR och senaste förendringen under 24 timmar är ' + \
                    cryptoValue[0]['percent_change_24h'] + '%'
